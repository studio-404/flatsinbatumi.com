...
class dashboard extends Controller
{
	...
	public function __construct()
	{
		...
		if($_SESSION[Config::SESSION_PREFIX."username"]=="root"){
			$this->managerNavigation->navigation = array(
				...
				$_SESSION["LANG"]."/dashboard/catalog/2"=>"კატალოგი", 
				$_SESSION["LANG"]."/dashboard/comments"=>"კომენტარები",
				$_SESSION["LANG"]."/dashboard/plugins"=>"პლაგინები",	
				$_SESSION["LANG"]."/dashboard/variants"=>"ვარიანტები",	
	...
	public function variants()
	{
		...
		// comments
		$variantsView = $this->model('variantsView');
		$variantsView->data = $getter;

		$this->view('dashboard/variants', [
			"header" => array(
				"website" => Config::WEBSITE,
				"public" => Config::PUBLIC_FOLDER
			),
			"theVariants" => $variantsView->index(), 
			"itemPerPage"=>$itemPerPage,
			"variants"=>$getter,
			"pagination"=>$pagination,
			"nav" => $this->managerNavigation->index(),
	...
	public function catalog()
	{
		...
		$s = (int)functions\request::index("GET","s");
