...
class page
{
	...
	private function select($args)
	{
		...
		$json = Config::CACHE."pages_".str_replace(array("-"," "), "", implode("_",$_SESSION['URL'])).$args['cid'].$args['nav_type'].".json";
		...
		foreach ($fetch as $val) {
			...
			if(count($args["serialPhotos"])){
				foreach ($args["serialPhotos"] as $pic) {
					...
					$fullPath = Config::DIR_.$pic;
					$getimagesize = getimagesize($fullPath);
					$size = sprintf("%sx%s", $getimagesize[0], $getimagesize[1]);

					$photo = 'INSERT INTO `photos` SET `parent`=:parent, `path`=:pathx, `type`=:type, `size`=:size, `lang`=:lang, `status`=:zero';
