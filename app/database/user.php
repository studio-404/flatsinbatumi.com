...
class user
{
	...
	private function updatepasswordWithRecoveryPassword($args){
		$sql = 'UPDATE `users_website` SET `password`=:password, `recoverpassword`=""  WHERE `recoverpassword`=:recoverpassword';
		$prepare = $this->conn->prepare($sql);
		$prepare->execute(array(
			":password"=>sha1(md5($args["password"])),
			":recoverpassword"=>sha1(md5($args["recoverpassword"]))
		));
		if($prepare->rowCount()){
			return true;
		}
